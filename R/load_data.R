...
# NUTS3 data -------------------------------------------------------------------
ap_socioecon_sf <- get(load('data/ap_socioecon_sf-Rev1.1.RData')) %>%
  dplyr::filter(!stringr::str_detect(geo, 'TR'))
deaths_socioecon_sf <- get(load('data/deaths_socioecon_sf-Rev1.1.RData')) %>%
  dplyr::filter(!stringr::str_detect(geo, 'TR'))
...
ap <- read.csv("data/EU_NECP_LTT-Rev1.1_2030_WORLD-NUTS3_pm25_avg.csv") %>%
  dplyr::rename(region = id_code,
                value = pm25_avg) %>%
  dplyr::distinct()

deaths <- get(load(paste0("data/necp_m3_get_mort_pm25-Rev1.1.output.RData"))) %>%
...
pm.ap_raster <- terra::rast("data/EU_NECP_LTT-Rev1.1_2030_pm25_fin_weighted.tif")
pm.mort_raster <- get(load(paste0("data/pm.mort_mat_2030",norm_grid_tag,"_EU_NECP_LTT-Rev1.1.RData"))); rm(pm.mort_yy); gc()
inc_pc_2015 <- terra::rast("data/Europe_disp_inc_2015.tif")
urbn_raster <- terra::rast("data/GHS_SMOD_E2030_GLOBE_R2023A_54009_1000_V2_0_reproj2.tif")
pop_ge65 <- terra::rast("data/ESTAT_OBS-VALUE-Y_GE65_2021_V2.tiff")
pop_t <- terra::rast("data/ESTAT_OBS-VALUE-T_2021_V2.tiff")
pm.weights_raster <- terra::rast('data/pm25_weights_rast_rfasstReg.tif')
